    ...
    <div v-if="showModal" class="modal modal-open">
        <div class="modal-box max-w-4xl">
            <div class="flex items-center justify-between">
                <h3 class="font-bold text-lg">{{ selectedService?.displayName }}</h3>
            ...
            <div class="space-y-4">
                <!-- Loading state -->
                <div v-if="selectedService?.isReloading" class="text-center py-8">
                    <span class="loading loading-spinner loading-lg"></span>
                    <p class="mt-4 text-base-content/70">Fetching service information...</p>
                </div>

                <!-- Service information -->
                <div v-else-if="selectedService?.info && !selectedService.info.error"
                ...
                <div v-else class="text-center py-4">
                    <p class="text-error font-medium">{{ selectedService?.info?.message || 'No information available for this service' }}</p>
                </div>
            </div>
