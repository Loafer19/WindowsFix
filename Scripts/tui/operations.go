...
func removeAllIcons() tea.Cmd {
	return func() tea.Msg {
		// Hide all desktop icons by setting policy
		regCmd := exec.Command("reg", "add", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", "/v", "NoDesktop", "/t", "REG_DWORD", "/d", "1", "/f")
		if err := regCmd.Run(); err != nil {
			return statusMsg("Error: Failed to hide desktop icons - " + err.Error())
		}
		...
		return statusMsg("Success: All desktop icons have been hidden.")
	}
}
